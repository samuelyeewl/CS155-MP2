...
class HiddenMarkovModel:
    ...
    def unsupervised_learning(self, X, iters):
        ...
        for iteration in range(iters):
            # Progress counter
            if float(iteration) / iters * 100 > percent:
                percent = float(iteration) / iters * 100
            ...
            for x in X:
                ...
                # for given (x, y)
                # First deal with probability of starting
                P_start_nxt = [0. for _ in range(self.L)]
                for nxt in range(self.L):
                    # Since alpha_0[start] = 1
                    P_start_nxt[nxt] = (1. * self.A_start[nxt] *
                                        self.O[nxt][x[0]] * betas[1][nxt])
                # Normalize:
                norm = sum(P_start_nxt)
                for nxt in range(self.L):
                    P_start_nxt[nxt] /= norm

                for nxt in range(self.L):
                    A_start_num[nxt] += P_start_nxt[nxt]

                for t in range(1, M):
                    P_curr_nxt = [[0. for _ in range(self.L)]
